...
import {
  ...
  PREVENTIVE_RESULT_OPTIONS, PREVENTIVE_MANAGER_OPTIONS, ACTIVE_ROUND_MAP
...
async function loadLevel5Users() {
  try { const { data } = await getLevel5UsersApi(); level5Users.value = data || [] } catch { /* handled */ }
}
...
async function handleTrack() {
  ...
  try { await trackLatentApi({ id: trackingRow.value.id, status: trackForm.status, remark: trackForm.remark }); ElMessage.success("操作成功"); trackDialogVisible.value = false; fetchData() } catch { /* handled */ } finally { submitting.value = false }
}
...
async function handleConfirmNotice(noticeId: number) {
  try { await ElMessageBox.confirm("确认接收此通知单吗？", "提示", { type: "info" }); await confirmNoticeApi(noticeId); ElMessage.success("已确认接收"); noticeDetailVisible.value = false; fetchData() } catch { /* cancelled */ }
}
async function viewNotice(row: any) {
  try { const { data } = await getNoticeListByBizApi(row.id, "latent"); if (data && data.length > 0) { noticeDetailData.value = data[0]; noticeDetailVisible.value = true } else { ElMessage.info("暂无通知单") } } catch { /* handled */ }
}
...
async function viewSupervision(row: any) {
  try { const { data } = await getSupervisionDetailApi(row.id); if (data) { supervisionDetailData.value = data; supervisionDetailVisible.value = true } else { ElMessage.info("暂无督导表") } } catch { /* handled */ }
}
...
async function handleSetMedication() {
  try { await setMedicationStatusApi({ id: medicationRow.value.id, medicationStatus: medicationStatusValue.value }); ElMessage.success("设置成功"); medicationDialogVisible.value = false; fetchData() } catch { /* handled */ }
}
  ...
  <div class="app-container">
    ...
    <el-card shadow="never">
      <template #header>
        <div class="flex items-center justify-between">
          <span class="text-lg font-bold">密接人群 — 潜伏感染管理</span>
    ...
    <el-dialog v-model="trackDialogVisible" title="追踪操作" width="450px">
      <el-form label-width="80px">
        <el-form-item label="追踪状态"><el-radio-group v-model="trackForm.status"><el-radio :value="1">到位</el-radio><el-radio :value="2">未到位</el-radio><el-radio :value="3">其他</el-radio></el-radio-group></el-form-item>
        <el-form-item v-if="trackForm.status === 3 || (trackForm.status === 2 && trackingRow?.notInPlaceCount >= 2)" label="备注原因"><el-input v-model="trackForm.remark" type="textarea" :rows="3" /></el-form-item>
      </el-form>
    ...
    <el-dialog v-model="xrayDialogVisible" title="录入胸片检查与诊断结果" width="520px">
      <el-alert type="info" :closable="false" class="mb-4" :description="`${xrayRow ? `${ACTIVE_ROUND_MAP[xrayRow.activeRound] || ''}轮次` : ''}追踪到位后，请录入胸片检查情况及诊断结果。`" />
    ...
    <el-dialog v-model="referralDialogVisible" title="转诊操作" width="450px">
      <el-form label-width="80px">
        ...
        <el-form-item v-if="referralForm.result === 'other'" label="备注原因"><el-input v-model="referralForm.remark" type="textarea" :rows="3" /></el-form-item>
      </el-form>
    ...
    <el-dialog v-model="noticeDialogVisible" title="填写潜伏者通知单" width="600px">
      <el-form :model="noticeForm" label-width="100px">
        ...
        <el-form-item label="性别"><el-select v-model="noticeForm.gender" style="width: 100%"><el-option label="男" value="男" /><el-option label="女" value="女" /></el-select></el-form-item>
        <el-form-item label="出生日期"><el-date-picker v-model="noticeForm.birthDate" type="date" value-format="YYYY-MM-DD" /></el-form-item>
        ...
        <el-form-item label="人群分类"><el-select v-model="noticeForm.crowdCategory" style="width: 100%"><el-option v-for="item in CROWD_CATEGORY_OPTIONS" :key="item" :label="item" :value="item" /></el-select></el-form-item>
        ...
        <el-form-item v-if="noticeForm.treatmentPlan === '个体化方案'" label="方案详情"><el-input v-model="noticeForm.customPlanDetail" type="textarea" :rows="3" /></el-form-item>
    ...
    <el-dialog v-model="supervisionDialogVisible" title="填写预防性治疗督导表" width="620px">
      <el-form :model="supervisionForm" label-width="130px">
        <el-form-item label="治疗开始日期"><el-date-picker v-model="supervisionForm.treatmentStartDate" type="date" value-format="YYYY-MM-DD" /></el-form-item>
        <el-form-item label="治疗方案"><el-select v-model="supervisionForm.treatmentPlan" style="width: 100%"><el-option v-for="item in TREATMENT_PLAN_OPTIONS" :key="item" :label="item" :value="item" /></el-select></el-form-item>
        <el-form-item label="督导内容"><el-input v-model="supervisionForm.supervisionContent" type="textarea" :rows="4" /></el-form-item>
        <el-divider>预防性治疗完成情况</el-divider>
        <el-form-item label="治疗完成时间"><el-date-picker v-model="supervisionForm.treatmentEndDate" type="date" value-format="YYYY-MM-DD" /></el-form-item>
        <el-form-item label="预防性治疗结果"><el-select v-model="supervisionForm.preventiveResult" clearable style="width: 100%"><el-option v-for="item in PREVENTIVE_RESULT_OPTIONS" :key="item" :label="item" :value="item" /></el-select></el-form-item>
        <el-form-item label="随访管理人员"><el-select v-model="supervisionForm.preventiveManager" clearable style="width: 100%"><el-option v-for="item in PREVENTIVE_MANAGER_OPTIONS" :key="item" :label="item" :value="item" /></el-select></el-form-item>
      </el-form>
    ...
    <el-dialog v-model="medicationDialogVisible" title="设置服药状态" width="450px">
      <el-form label-width="100px">
        <el-form-item label="服药状态"><el-radio-group v-model="medicationStatusValue"><el-radio v-for="item in MEDICATION_STATUS_OPTIONS" :key="item.value" :value="item.value">{{ item.label }}</el-radio></el-radio-group></el-form-item>
      </el-form>
    ...
    <el-dialog v-model="treatmentDialogVisible" :title="`预防治疗管理 — ${treatmentRow?.name || ''}`" width="800px">
      <el-tabs>
        <el-tab-pane label="电话随访">
          <div class="mb-3 flex justify-end"><el-button type="primary" size="small" v-permission="'latent:followUp'" @click="openFollowUpForm">新增电话随访</el-button></div>
        ...
        <el-tab-pane label="按期检查">
          <div class="mb-3 flex justify-end"><el-button type="primary" size="small" v-permission="'latent:check'" @click="openCheckForm">新增按期检查</el-button></div>
    ...
    <el-dialog v-model="followUpFormVisible" title="新增电话随访" width="500px" append-to-body>
      <el-form :model="followUpForm" label-width="80px">
        <el-form-item label="随访日期"><el-date-picker v-model="followUpForm.followUpDate" type="date" value-format="YYYY-MM-DD" /></el-form-item>
    ...
    <el-dialog v-model="checkFormVisible" title="新增按期检查" width="500px" append-to-body>
      <el-form :model="checkForm" label-width="80px">
        <el-form-item label="检查日期"><el-date-picker v-model="checkForm.checkDate" type="date" value-format="YYYY-MM-DD" /></el-form-item>
    ...
    <el-dialog v-model="aggregateDialogVisible" title="潜伏感染者信息归集" width="750px" destroy-on-close>
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="阳性轮次">{{ ACTIVE_ROUND_MAP[aggregateRow?.activeRound] || "-" }}</el-descriptions-item>
