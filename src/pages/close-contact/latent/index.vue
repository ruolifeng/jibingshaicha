...
function handleReset() {
  searchForm.name = ""; searchForm.idNumber = ""; searchForm.archived = undefined
  handleSearch()
}
...
async function handleSendNotice() {
  ...
  try {
    ...
    await sendNoticeApi({
      noticeType: "latent", populationType: POPULATION_TYPE,
...
const supervisionForm = reactive({
  ...
  supervisionContent: "", preventiveResult: "", preventiveManager: ""
})
...
function openMedicationDialog(row: any) {
  medicationRow.value = row; medicationStatusValue.value = row.medicationStatus || 1; medicationDialogVisible.value = true
}
...
async function handleSetMedication() {
  try {
    ...
    ElMessage.success("服药状态设置成功"); medicationDialogVisible.value = false; fetchData()
  } catch { /* handled */ }
}
...
async function loadFollowUps(id: number) {
  try { const { data } = await getFollowUpListApi(id); followUpList.value = data || [] } catch { /* handled */ }
}

async function loadChecks(id: number) {
  try { const { data } = await getCheckListApi(id); checkList.value = data || [] } catch { /* handled */ }
}
...
function openFollowUpForm() {
  Object.assign(followUpForm, { followUpDate: "", content: "", result: "" }); followUpFormVisible.value = true
}

async function handleSaveFollowUp() {
  try {
    await saveFollowUpApi({ latentInfectionId: treatmentRow.value.id, ...followUpForm, operator: userStore.realName || userStore.username })
    ElMessage.success("随访记录保存成功"); followUpFormVisible.value = false; loadFollowUps(treatmentRow.value.id)
  } catch { /* handled */ }
}
...
function openCheckForm() {
  Object.assign(checkForm, { checkDate: "", checkPeriod: "", checkResult: "", content: "" }); checkFormVisible.value = true
}

async function handleSaveCheck() {
  try {
    await saveCheckApi({ latentInfectionId: treatmentRow.value.id, ...checkForm, operator: userStore.realName || userStore.username })
    ElMessage.success("检查记录保存成功"); checkFormVisible.value = false; loadChecks(treatmentRow.value.id)
  } catch { /* handled */ }
}
...
async function handleCloseCase(row: any) {
  try {
    ...
    ElMessage.success("结案归档成功"); fetchData()
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="tableData" border stripe max-height="600">
        ...
        <el-table-column label="阳性轮次">
          <template #default="{ row }">{{ ACTIVE_ROUND_MAP[row.activeRound] || "-" }}</template>
        </el-table-column>
