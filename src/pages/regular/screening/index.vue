...
async function handleUpload(uploadFile: any) {
  try {
    const data = await runImportWithIdentityConfirm(uploadScreeningRegularApi, uploadFile.raw)
    if (!data) return
...
async function handleDeleteFiltered() {
  ...
  try {
    const { data } = await deleteScreeningRegularByFilterApi(buildListQueryParams())
...
async function handleDeleteAll() {
  ...
  batchDeleting.value = true
  try {
    const { data } = await deleteAllScreeningRegularApi()
    ElMessage.success(`成功删除 ${data ?? 0} 条记录`)
    selectedRows.value = []
    handleReset()
  } catch (err: any) {
    ...
    <el-card shadow="never">
      <template #header>
        <div class="flex items-center justify-between">
          <span class="text-lg font-bold">疫情筛查数据</span>
          <div class="flex gap-2 flex-wrap">
            <el-button v-permission="'regular:screening:create'" type="success" @click="handleCreate">
              新增数据
            </el-button>
            <el-button v-permission="'regular:screening:export'" :loading="exporting" @click="() => handleExport('filtered')">
              导出筛选结果
            </el-button>
            <el-button v-permission="'regular:screening:delete'" type="danger" plain :loading="batchDeleting" @click="handleDeleteFiltered">
              删除筛选结果
            </el-button>
            <el-button v-permission="'regular:screening:export'" type="warning" :loading="exporting" :disabled="selectedRows.length === 0" @click="handleExportSelected">
              导出勾选
            </el-button>
            <el-button v-permission="'regular:screening:delete'" type="danger" :loading="batchDeleting" :disabled="selectedRows.length === 0" @click="handleBatchDelete">
              删除勾选
            </el-button>
            <el-upload
              ref="uploadRef"
            ...
            >
              <el-button v-permission="'regular:screening:upload'" type="primary">
                上传 Excel
              </el-button>
            </el-upload>
            <el-button v-permission="'regular:screening:export'" :loading="exporting" @click="() => handleExport('all')">
              导出全部
            </el-button>
            <el-button v-permission="'regular:screening:delete'" type="danger" plain :loading="batchDeleting" @click="handleDeleteAll">
              删除全部
            </el-button>
          </div>
        </div>
