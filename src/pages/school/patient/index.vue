...
async function handleImportEpidemic(uploadFile: any) {
  try {
    const { data } = await importEpidemicApi(uploadFile.raw, "school")
          ...
          <template #default="{ row }">
            <div class="action-btns">
              ...
              <el-dropdown trigger="click" @command="(cmd: string) => handleActionCommand(cmd, row)">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item v-permission="'patient:followUp'" command="followUp">
                      填写后续随访表
                    </el-dropdown-item>
                    <el-dropdown-item command="followUpList">
                      随访记录
                    </el-dropdown-item>
                    <el-dropdown-item v-permission="'patient:medication'" command="medication" divided>
                      填写服药管理
                    </el-dropdown-item>
    ...
    <el-dialog v-model="followUpDialogVisible" title="填写后续随访记录" width="560px">
      <el-form :model="followUpForm" label-width="100px">
        ...
        <el-form-item label="随访情况">
          <el-input v-model="followUpForm.visitSituation" type="textarea" :rows="4" placeholder="请填写本次随访情况" />
        </el-form-item>
