#!/bin/bash
set -euo pipefail
FRONTEND_TAR="${1:?frontend tar required}"
...
cp -a /tmp/admin-1.0-SNAPSHOT.jar admin-to-encrypt.jar
...
rm -f "$FRONTEND_TAR" /tmp/admin-1.0-SNAPSHOT.jar \
  /root/deploy/backend/admin-to-encrypt.jar \
  /root/deploy/backend/admin-to-encrypt-encrypted.jar 2>/dev/null || true
