...
CREATE TABLE IF NOT EXISTS `user` (
    ...
    `phone`       VARCHAR(20)  DEFAULT NULL COMMENT '联系电话（系统消息短信接收号）',
...
CREATE TABLE IF NOT EXISTS `medication_management` (
    ...
    `order_stop_periods`      JSON         DEFAULT NULL COMMENT '医嘱停药多段 [{startDate,endDate,reason}]',
...
CREATE TABLE IF NOT EXISTS `medication_pickup` (
    ...
    `entry_unit`        VARCHAR(200) DEFAULT NULL COMMENT '录入单位',
    `entry_person`      VARCHAR(100) DEFAULT NULL COMMENT '录入人员',
    `remarks`           TEXT         DEFAULT NULL COMMENT '备注',
    `filled_by`         BIGINT       DEFAULT NULL COMMENT '填写人ID',
    `create_time`       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `update_time`       DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- ==================== V91：筛查表历史录入用户名回填（详见 migration/V91_screening_creator_username_backfill.sql） ====================
-- 有 creator_id 补 username；空录入人时从潜伏感染或部门唯一五级用户代填
...
WHERE sk.deleted = 0 AND sk.creator_id IS NULL;
...
WHERE ss.deleted = 0 AND ss.creator_id IS NULL;
...
WHERE sc.deleted = 0 AND sc.creator_id IS NULL;
...
WHERE s.deleted = 0 AND s.creator_id IS NULL AND s.department_id IS NOT NULL;
...
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 501, 'questionnaire:list', '问卷列表', 1, 500, 1
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:list');
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 502, 'questionnaire:template:view', '问卷模板', 1, 500, 2
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:template:view');
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 510, 'questionnaire:create', '创建问卷', 2, 500, 10
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:create');
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 511, 'questionnaire:update', '编辑问卷', 2, 500, 11
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:update');
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 512, 'questionnaire:delete', '删除问卷', 2, 500, 12
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:delete');
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 513, 'questionnaire:publish', '发布问卷', 2, 500, 13
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:publish');
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 514, 'questionnaire:data', '问卷数据', 2, 500, 14
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:data');
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 515, 'questionnaire:data:export', '数据导出', 2, 500, 15
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:data:export');
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 516, 'questionnaire:template:delete', '删除模板', 2, 500, 16
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:template:delete');

INSERT IGNORE INTO `role_permission` (`id`, `role`, `permission_id`)
SELECT (500000 + r.role * 100 + (p.id - 500)), r.role, p.id
FROM (SELECT 1 AS role UNION SELECT 2 UNION SELECT 3 UNION SELECT 4) r
         CROSS JOIN `permission` p
WHERE p.`code` LIKE 'questionnaire%'
  AND NOT EXISTS (
...
INSERT INTO `permission` (`id`, `code`, `name`, `type`, `parent_id`, `sort`)
SELECT 503, 'questionnaire:category', '问卷分类', 1, 500, 3
WHERE NOT EXISTS (SELECT 1 FROM `permission` WHERE `code` = 'questionnaire:category');
