import { resolveRouteTagTitle, type RouteTagTitleMatch, type RouteTagTitleSource } from "@@/utils/route-tag-title"
...
describe("resolveRouteTagTitle", () => {
  ...
  it("已配置带连字符的 tagTitle 优先使用", () => {
    const route = mockRoute(
      { title: "在管总览", tagTitle: "潜伏-在管总览" },
      [
        { name: "LatentManagement", meta: { title: "潜伏感染者管理" } },
        { meta: { title: "在管总览" } }
      ]
    )
    expect(resolveRouteTagTitle(route)).toBe("潜伏-在管总览")
  })
